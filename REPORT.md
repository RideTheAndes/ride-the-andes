...
**Date:** 2026-08-03 · **Revised:** 2026-08-04 after an independent review (see `VERIFICATION.md` for what that review got right and what did not reproduce)
**Scope:** Full repository — every `.html` file, `assets/css/styles.css`, `assets/js/site.js`, `robots.txt`, `sitemap.xml`, `_headers`, `_redirects`, and every image actually referenced by the site — inspected directly, not assumed. Findings below are backed by things checked in this repo: cross-referencing every image filename against all HTML *plus* the stylesheet and script, a JPEG-header parser for real image dimensions/weights, a WCAG contrast-ratio calculator run against the actual CSS custom properties, a JSON-LD parser run against every structured-data block, and an `Object.keys()` count of the live `ES` dictionary for the i18n figures.

This audit produced four fix PRs (one per category, as requested) plus this report:

| PR | Category | Branch | State |
|---|---|---|---|
| #6 | Technical SEO | `claude/seo-technical-fixes` | open |
| #7 | Structured data | `claude/structured-data` | **merged** |
| #8 | Accessibility | `claude/accessibility-fixes` | open |
| #9 | Performance | `claude/performance-fixes` | open — **stacked on #8**, see `VERIFICATION.md` |
...
| 1.3 | `sitemap.xml` had no `<lastmod>` on any URL | Medium | Google uses `lastmod` as a freshness/recrawl signal | Low | **Fixed** (PR #6). Revised after review: the first pass stamped one identical date on all five URLs, which is a false freshness signal; now per-page and derived from each page's actual last content change |
| 1.4 | `/index.html` and `/journal/index.html` are reachable at both their clean path and their literal filename, with no redirect consolidating them (canonical tags already existed, but the redirect layer didn't match) | Low | Minor duplicate-URL/link-equity dilution | Low | **Fixed** (PR #6). Verified these 301s are *not* redundant with Netlify's built-in pretty-URL handling — Netlify does not canonicalise `/path/index.html` → `/path/`, because that URL maps to a real file on disk |
| 1.4b | The pre-existing `/journal → /journal/index.html 200` rewrite served a full duplicate 200 response at `/journal`, competing with `/journal/` — the URL the canonical tag and sitemap both declare — and preempting Netlify's own directory redirect | Low | Duplicate-URL signal working against the §1.4 canonicalisation | Low | **Fixed** (PR #6) — changed to a 301 so the URL keeps resolving and consolidates onto the canonical form |
...
| 1.5b | The new `404.html` linked its stylesheet **relatively**. Netlify serves the 404 body at the *requested* URL, so a broken link at `/journal/foo/bar` resolved the stylesheet to `/journal/foo/assets/css/styles.css` → an unstyled error page at any nested path | **High** | Every deep broken link rendered as raw unstyled HTML | Low | **Fixed** (PR #6) — all `href`/`src` in `404.html` are now absolute. The other pages keep relative paths on purpose: they are served from their own fixed URL, where relative resolution is correct |
...
| # | Finding | Priority | Impact | Effort | Status |
|---|---|---|---|---|---|
| 2.1 | **`index.html` ships 19 CSS `background-image` references against exactly 1 `<img>` tag — a 19:1 ratio — and that single `<img>` carries `alt=""`.** Counting the 2 further `<img>` elements that `site.js` injects for the hero rotation (also `alt=''`), the page pulls in 22 image files totalling **5.99 MB**, of which **zero** are indexable images with descriptive alt text. Google Images does not index CSS background images | **High** | The site's strongest asset — curated Boyacá/cycling photography — earns essentially zero Google Images traffic, a real channel for "Colombia cycling tour" / "Boyacá" visual search intent | High (structural — see §7) | Partially mitigated: `role="img"` + `aria-label` added to all 19 (PR #8), so the content is at least described to assistive tech and to crawlers reading the DOM, and those labels now follow the EN/ES toggle. A full fix needs real `<img>` tags with `alt` — see §7 |
...
| 2.4 | Title tags are already reasonably keyword-rich (location + product terms) across all pages; no rewrite attempted since these are marketing copy, not purely technical fields | Low | — | — | Recommendation only |

## 3. International SEO

This is the single highest-impact finding of the audit — see **§1.7** above and the dedicated recommendation in **§7, item 1**. Summary: the EN/ES toggle is a client-side `innerHTML` swap with no separate URL per language, so **no valid `hreflang` markup can be added today** — `hreflang` requires each language variant to live at its own indexable URL, and adding it pointing at the same URL twice would be actively incorrect per Google's guidelines. We did not add fake `hreflang` tags. German scalability inherits the same problem: bolting on a third `DE` dictionary to the existing toggle would just make three languages invisible to search instead of one.

**Translation coverage is not a blocker for this work — verified.** A follow-up review asserted the ES dictionary was missing ~105 of ~308 keys (~34% of the page falling back to English, including the whole nav). That does not reproduce. Parsing `data-i18n` out of `index.html` and the `ES` object out of `site.js` — and independently confirming by `eval`-ing the object in Node and reading `Object.keys().length` — gives:

| Measure | Count |
|---|---|
| Unique `data-i18n` keys in `index.html` (at time of review) | 308 |
| Keys in the `ES` dictionary | 309 |
| Keys used in HTML with **no** ES translation | **0** |
| ES keys not used by any element | 1 (`pr_foot`) |

Every key named in that review as missing (`nav_route`, `nav_itin`, `nav_incl`, `nav_guide`, `nav_price`, `nav_cta`, `ff_country`, `ff_email`, `s1_day`, `s1_grade`, `s_view1`, `d1_tag`, `jt1_h`, `ft_h1`) is present with a real Spanish string. The "~204" figure is reproducible as a **counting artifact**: `grep -cE '^\s*\w+\s*:'` returns exactly 204 because it counts only the first key on each line, and 50 lines of the `ES` object declare more than one key (e.g. the nav line declares seven). The Spanish body copy is complete.

What *was* genuinely untranslated: the ~21 `aria-label` accessible names and the "Skip to content" link added by the accessibility pass, which `setLang()` could not reach because it only swaps `innerHTML`. Fixed in PR #8 via a `data-i18n-aria` hook — see §6.12. `pr_foot` is a real orphan and is left in place rather than deleted: its `.price-foot` CSS rule still exists, so it may be intended for a pricing element not currently rendered. Flagged for the operator.
...
| 4.7 | `publisher.logo` on the `BlogPosting` schema — Google's Article guidelines want a logo `ImageObject`, but there's no dedicated square/rectangular logo file in the repo (only an inline SVG brandmark and a 1200×630 landscape `og-image.jpg`, which is the wrong aspect ratio to pass off as a logo) | Low | Recommendation only — see §7, item 5 |
...
| 5.5 | **21 of the 47** files in `assets/img/` (`stat-*`, most `culture-*`, `poi-etapafinal-*`, `hero-1..4-*` legacy variants, `your-guide.jpg`) are **not referenced by any HTML page, by `styles.css`, or by `site.js`** — 20.7 MB of the directory's 29.1 MB total | Low | Zero performance impact (never fetched by visitors) — pure repo hygiene | Low | Not touched; recommend archiving or deleting if truly unused. See §7 for the `your-guide.jpg` open question |
...
| 6.12 | The ~21 `aria-label` accessible names and the "Skip to content" link added by this audit were English-only in ES mode — `setLang()` only swaps `innerHTML` on `[data-i18n]`, so it could not reach attribute values. A Spanish screen-reader user heard Spanish body copy narrated with English image descriptions | Medium | **Fixed** — added a `data-i18n-aria` hook that captures English labels into `EN_ARIA` (mirroring the existing `EN` capture) and swaps `aria-label` from the same `ES` dictionary. All 21 translatable labels plus the skip link wired; the two brandmark SVGs stay untagged since "Ride The Andes" is a proper noun. Round-trip EN→ES→EN tested |

---

## 7. Prioritized Action Plan (remaining work, in priority order)
...
1. **Critical — Real Spanish (and future German) URLs with `hreflang`.** The single biggest opportunity found. Recommended approach: generate `/es/index.html` (and eventually `/de/index.html`) as real static pages using the translated copy that already exists in `site.js`'s `ES` object — it just needs to be server-rendered instead of JS-swapped — plus reciprocal `hreflang="en"`/`hreflang="es"`/`hreflang="x-default"` tags on both versions and a sitemap update. This is a genuine content/architecture project, not a one-line fix, and should go through the same Netlify-preview review as any `index.html` change. Estimated effort: 1–2 days including QA.
   - **Sequencing check, resolved:** a reasonable concern is that generating `/es/` from a partially-filled dictionary would ship a static Spanish page that is really a third English. That concern was checked and does **not** apply here — the `ES` dictionary covers **100%** of the `data-i18n` keys `index.html` uses (0 unresolved; see §3 for the derivation). This item is therefore *unblocked today*. The one prerequisite worth keeping is procedural, not content: the generator must fail loudly on a missing key rather than silently emitting the English fallback, so the page can never regress to mixed-language without someone noticing. A CI check asserting `keys(index.html) ⊆ keys(ES)` is the cheap version of that, and would also have prevented the mistaken "34% untranslated" reading in the first place.
...
8. **Low — Repo hygiene**: 21 unreferenced images (20.7 MB of the 29.1 MB in `assets/img/`) with no page, stylesheet, or script pointing to them. Confirm with the operator whether they're reserved for planned journal posts before deleting. Resolve the `your-guide.jpg` question in §7 first.
9. **Open question for the operator — `assets/img/your-guide.jpg`.** This file has never been referenced by any HTML, CSS, or JS in *any* commit in the repository's history (`git log --all -S'your-guide.jpg'` returns nothing); it entered in commit `f102960 "add files via upload"` and was never wired up. Meanwhile the "Your Guide" section renders `journal-123-hero-sergio-diego.jpg` — a photo containing two riders — and `styles.css` carries a hand-tuned `background-position:62% center` with the comment *"el encuadre cae sobre Sergio — ciclista de la DERECHA (el de rojo es Diego)"*, i.e. the framing is a workaround to crop a second person out of frame. That is consistent with `your-guide.jpg` being the intended dedicated portrait that was never connected. **Do not delete it as part of the §7 item 8 cleanup** until the operator confirms whether the guide section was meant to use it. If it was, wiring it in is a small change that also removes the crop hack.
10. **Content opportunity — prioritize the two already-planned Journal posts** that have the clearest search intent and lowest competition: *"Cycling at Altitude in Colombia: How to Acclimatise"* and *"Is It Safe to Cycle in Colombia? An Honest Local Answer"* (both already listed as "Coming soon" on `journal/index.html`). These map directly to real traveler-intent queries (altitude, safety) that aren't yet answered anywhere on the site outside of a couple of FAQ lines. A few additional FAQ/content gaps worth considering, to be verified by the operator before publishing (none of these facts are asserted here as true — they're topic suggestions, not answers): Colombian visa requirements for the traveler's likely source countries, airline bike-transport policies, and currency/tipping norms.
...
- **6–12 months**: the highest-leverage item by far is the international SEO fix (§7, item 1) — if implemented, it opens the entire Spanish-language search market for Boyacá/Colombia cycling tourism that the site currently cannot rank for at all, which is plausibly a larger opportunity than everything else in this report combined, given the business's own customer base (Colombian Gran Fondo riders) skews Spanish-speaking. The Journal content expansion (§7, item 10) is the second-highest-leverage item, since long-tail informational queries ("cycling at altitude," "is it safe to cycle in Colombia") are exactly where a small operator can realistically outrank larger competitors.
